#!/usr/bin/env bash
# Strategic-merge contention live gate (node_disk_v2, workload-125-225).
# Usage: run_strategic_merge_contention_live_gate_one.sh <knative|mlp|gnn> <config_name> <config_json_path>
set -euo pipefail
...
GNN_MODEL="${GNN_MODEL:-models/near-rtt-v2-strategic-merge-wss-cont-v2-dim14-ce-only.pt}"
MLP_MODEL="${MLP_MODEL:-models/tabular/batch_edge_mlp_strategic_merge_wss_cont_v2_dim22_batchcache.pt}"
...
echo "=== strategic-merge contention live gate: ${POLICY} / ${CONFIG_NAME} ==="
