...
|---|---|---|
| [**objective_pivot_v1**](docs/lineages/objective_pivot_v1.md) | `ACTIVE` | **Current work. Program pivot registered 2026-08-28: stop engineering the environment, change the training objective.** **Phase 1 CLOSED 2026-08-29 — PASS.** The 16-seeded-draw reliability gate ran at the pinned commit `c08aa7e` (all 8 new arms provenance-verified, 240/240 cells): GNN severe-collapse burden is stochastically smaller than the frozen MLP's — primary rank-sum **p = 0.00143** at +50%, must-hold **p = 0.00045** at +100% (GNN clean in 16/16 draws vs MLP up to 23 collapsed cells). Secondary also passes: mean margin vs Knative negative in **16/16** draws (p = 1.5e-05). **Registered scope limit: severe collapse only** — at +30% two GNN draws still carry real burden, so the claim is "tight distribution with a tail vs a lottery", never "never collapses" (`gnn_draw_study_v1` falsified that). Operational: the gate lost 191/240 tasks twice to an exhausted per-account `/home` quota (0-byte SLURM logs, `df` misleading); freed by deleting the PARKED route_b dataset dirs (588 datasets, 55G, none registered). Next: Phase 2 = P3 horizon pilot as registered in `program_verdict_v1`; Phase 3 = P1 closed-loop training — the one path `program_verdict_v1` left open to the latency claim. |
| [**mp_ablation_v1**](docs/lineages/mp_ablation_v1.md) | `REGISTERED` | **Control on the program's only established claim, registered 2026-08-29 before any run.** `objective_pivot_v1` Phase 1 worded its reliability result as being about *graph-awareness*, but GNN and MLP differ in architecture, capacity and optimiser path too. Asks whether **message passing** is the cause, via a **paired** design: `GNN_DISABLE_MESSAGE_PASSING=1` skips the GIN forward call while leaving the parameter set and its init identical, so at a fixed seed the two arms start from bit-identical weights (16 pairs, same pinned commit `c08aa7e`, same 30 cells). Primary = paired Wilcoxon on per-seed mean margin vs Knative; co-primary = paired sign test on severe-collapse counts over non-tied pairs, with ties expected and declared in advance. **A null result is a failure to detect, never proof of equivalence.** If message passing is not load-bearing, the Phase 1 claim must be reworded away from "graph-aware". |
