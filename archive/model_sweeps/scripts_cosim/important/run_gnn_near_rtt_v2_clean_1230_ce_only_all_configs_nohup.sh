...
OUT_ROOT="${1:-simulation_data/normal_sim_sweeps/gnn_near_rtt_v2_clean_1230_ce_only_20260608}"
TS="$(basename "$OUT_ROOT" | grep -oE '[0-9]{8}(_[0-9]{6})?' | tail -1 || date +%Y%m%d_%H%M%S)"
CFG_DIR="${OUT_ROOT}/configs"
RES_DIR="${OUT_ROOT}/results"
PROGRESS_LOG="logs/gnn_near_rtt_v2_clean_1230_ce_only_sweep_${TS}.log"
...
MODEL_PATH="models/near-rtt-v2-clean-1230-ce-only.pt"
TIMEOUT=3600
SEED=42
...
log "=== GNN near-rtt-v2-clean-1230-ce-only sweep start ==="
log "Model: ${MODEL_PATH}"
log "Decode: argmax (default)"
log "Output: ${OUT_ROOT}"
...
log "=== GNN near-rtt-v2-clean-1230-ce-only sweep complete ==="
