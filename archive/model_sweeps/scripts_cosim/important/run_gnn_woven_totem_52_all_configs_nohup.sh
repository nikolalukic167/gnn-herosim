...
OUT_ROOT="${1:-simulation_data/normal_sim_sweeps/gnn_woven_totem_52_20260607}"
TS="$(basename "$OUT_ROOT" | grep -oE '[0-9]{8}_[0-9]{6}' || date +%Y%m%d_%H%M%S)"
CFG_DIR="${OUT_ROOT}/configs"
RES_DIR="${OUT_ROOT}/results"
PROGRESS_LOG="logs/gnn_woven_totem_52_sweep_${TS}.log"
...
MODEL_PATH="models/woven-totem-52.pt"
TIMEOUT=3600
SEED=42
...
log "=== GNN woven-totem-52 sweep start ==="
...
log "=== GNN woven-totem-52 sweep complete ==="
