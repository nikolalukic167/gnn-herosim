#!/usr/bin/env bash
# Push warmth_v2 regen code + local progress to datalab; submit 4-shard SLURM array.
set -euo pipefail
...
rsync -avP -e "$RSYNC_SSH" \
  ...
  scripts_cosim/datalab/submit_warmth_regen_remaining.sh \
...
# All completed datasets so --resume skips overlap on datalab
if [[ -d "${LOCAL_OUT}" ]]; then
  rsync -avP -e "$RSYNC_SSH" \
    "${LOCAL_OUT}/" \
...
echo "    Submitting SLURM array on datalab..."
ssh -i "$SSH_KEY" "$REMOTE" "cd ${REPO} && bash scripts_cosim/datalab/submit_warmth_regen_remaining.sh"
