...
OUT_ROOT="${1:-simulation_data/normal_sim_sweeps/gnn_near_rtt_v2_clean_1230_seqblend_m2_20260608}"
TS="$(basename "$OUT_ROOT" | grep -oE '[0-9]{8}(_[0-9]{6})?' | tail -1 || date +%Y%m%d_%H%M%S)"
CFG_DIR="${OUT_ROOT}/configs"
RES_DIR="${OUT_ROOT}/results"
PROGRESS_LOG="logs/gnn_near_rtt_v2_clean_1230_seqblend_m2_sweep_${TS}.log"
...
MODEL_PATH="models/near-rtt-v2-clean-1230.pt"
TIMEOUT=3600
SEED=42
SEQBLEND_MARGIN="${GNN_SEQBLEND_QUEUE_MARGIN:-2}"

export GNN_MODEL_PATH="${MODEL_PATH}"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"
export GNN_DECODE_MODE="seqblend"
export GNN_SEQBLEND_QUEUE_MARGIN="${SEQBLEND_MARGIN}"

log() {
  echo "[$(date -Is)] $*"
}

log "=== GNN near-rtt-v2-clean-1230 seqblend m${SEQBLEND_MARGIN} sweep start ==="
log "Model: ${MODEL_PATH}"
log "Decode: seqblend margin=${SEQBLEND_MARGIN}"
log "Output: ${OUT_ROOT}"
...
log "=== GNN near-rtt-v2-clean-1230 seqblend m${SEQBLEND_MARGIN} sweep complete ==="
