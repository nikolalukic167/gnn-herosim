#!/usr/bin/env bash
# Push skew_warmth_v2 regen code to datalab and submit 4-shard SLURM array.
set -euo pipefail
...
OUTPUT_SUBDIR="${OUTPUT_SUBDIR:-gnn_datasets_4tasks_skew_warmth_v2}"
...
rsync -avP -e "$RSYNC_SSH" \
  ...
  scripts_cosim/datalab/submit_skew_warmth_v2_regen.sh \
...
echo "    Submitting skew_warmth_v2 SLURM array on datalab..."
ssh -i "$SSH_KEY" "$REMOTE" \
  "cd ${REPO} && sed -i 's/\r$//' scripts_cosim/datalab/*.sh scripts_cosim/datalab/*.sbatch && \
  bash scripts_cosim/datalab/submit_skew_warmth_v2_regen.sh"
