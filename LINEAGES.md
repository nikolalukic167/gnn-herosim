...
|---|---|---|
| [**peer_affinity_v1**](docs/lineages/peer_affinity_v1.md) | `REGISTERED` | **Registered 2026-09-09, before any number.** The one enumeration-compatible structure the count argument does not cover: a cost indexed by *pairs of task instances* (continuous, instance-specific exchange volumes x_ij, no commit order) under binding capacity — a quadratic assignment. Phase 0 is a paper screen on `arm_b0`-derived paper datasets (60 cells × 34) with registered bars: S0a/S0b controls, B0 cap binds ≥ 50 %, B1 joint gap ≥ 5 %, B2 count repair < 0.5, **B3 prefix-greedy regret ≥ 5 %**, B4 peer-mass closure < 0.5, **B5 hand-lookahead greedy ≥ 2 % (kill bar)**, C1 count-oracle ≥ 5 %; spread control is VOID at k > n_nodes and is never read as a pass. GO builds `HEROSIM_PEER_EXCHANGE=1`; NO-GO files a hard-stop with no code. `scripts_cosim/peer_affinity_probe.py`. |
