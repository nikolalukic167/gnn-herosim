...
"""Train dim14 CE-only from scratch on contention_v3 cache (900 ds)."""
...
def main() -> None:
    ...
    cache = Path(
        os.environ.get(
            "NEAR_RTT_CACHE_DIR",
            str(repo / "simulation_data" / "graphs_cache_contention_v3"),
        )
    )
    if not cache.is_dir():
    ...
    os.environ.setdefault("WANDB_RUN_NAME", "near-rtt-v2-contention-v3-dim14-ce-only")
    os.environ.setdefault(
        "WANDB_TAGS",
        "near-rtt,ce-only,dim14,contention-v3,from-scratch,collision-labels",
    )
    ...
    sys.argv = [
        ...
        "--wandb-project",
        os.environ.get("WANDB_PROJECT", "gnn-near-rtt-contention-v3-jun2026"),
    ]
