...
|---|---|---|
| [**peer_affinity_v1**](docs/lineages/peer_affinity_v1.md) | `ACTIVE` | **T1 read (2026-09-10): GNN-NEEDED over peer-blind pointwise (+13.4 pp, p = 0.0078, 8/8 seeds), but message passing is NOT the lever.** The first environment in the program whose joint structure defeats a *trained* pointwise scorer: a cost indexed by pairs of task instances (continuous exchange volumes, no commit order) under a binding cap. `mlp_t1` (sees exact cost to committed peers) loses 11-13 pp to every arm that also sees the uncommitted-peer lookahead column. But `mpoff` (message passing disabled, `PeerConv` never runs) ties the full GNN (+0.77 pp, p = 0.31), and a pointwise MLP with that one hand-built scalar lands within 2.7 pp (p = 0.31) - **hand lookahead suffices**. Both secondary contrasts are INDETERMINATE, not ties: point estimates sit at the registered ~2 pp effect and n = 8 cannot resolve them (~31 / ~16 seeds needed). Fit ceiling: the GNN memorises the train split exactly (0.00 % regret) and still reads 24.96 % held-out. Upstream: paper screen GO (14/60), physics `HEROSIM_PEER_EXCHANGE=1` bit-identical off, simulated rung R2 blind-passed every bar. 192 runs, 0 failures, 0 infeasible decodes in 2 176 held-out plans. **Open: a powered rerun of `gnn` vs `mpoff`/`mlp_t1x`.** `scripts_cosim/peer_affinity_t1_read.py`. |
