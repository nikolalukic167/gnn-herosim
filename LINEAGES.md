...
| [**route_b_v1**](docs/lineages/route_b_v1.md) | `ACTIVE` | **Stage 1 PASS** — contention + coupling produces the non-pointwise structure five mechanisms and route A could not. **Stage 2 NO-GO-PREPROBE** (2026-08-26) — its premise failed a convergence check (2026-09-03: A1 ran 40 epochs; at 300 its train regret goes 28.45% → ~1%). **Fit-ceiling Phase 1 (2026-09-06, 8 seeds):** the GNN fits the DAG training pipelines 4–10× tighter than pointwise and still loses held-out by a few points (median-paired MP-OFF−GNN −3.6 pp, p=0.031, exploratory); the single-seed 2–3× gap was a draw. **Phase 2 REGISTERED 2026-09-06:** a learning curve — train 204 / 612 / 1020 DAG datasets against one fixed 204-parent held-out block, three arms × 8 seeds; primary = median-paired GNN−MP-OFF at rung 3 (DATA-RESCUE / GAP-PERSISTS / INDETERMINATE). Fork `route_b_env_pivot_v1` stays PARKED. |
