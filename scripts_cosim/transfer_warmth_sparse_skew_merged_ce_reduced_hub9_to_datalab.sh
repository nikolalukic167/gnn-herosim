#!/usr/bin/env bash
# Push warmth+sparse+skew merged ce-reduced hub9 sweep (27 jobs) to datalab and submit.
set -euo pipefail
...
SWEEP_DIR="${SWEEP_DIR:-simulation_data/normal_sim_sweeps/warmth_sparse_skew_merged_ce_reduced_hub9_20260612}"
GNN_MODEL="${GNN_MODEL:-models/near-rtt-v2-warmth-sparse-skew-merged-ce-reduced.pt}"
...
rsync -avP -e "$RSYNC_SSH" \
  ...
  scripts_cosim/important/prepare_warmth_sparse_skew_merged_hub9_sweep.sh \
...
ssh -i "$SSH_KEY" "$REMOTE" "chmod +x \
  ...
  ${REPO}/scripts_cosim/important/prepare_warmth_sparse_skew_merged_hub9_sweep.sh && \
...
submit_out=$(ssh -i "$SSH_KEY" "$REMOTE" "cd ${REPO} && \
  ...
  bash scripts_cosim/datalab/submit_warmth_sparse_merged_ce_reduced_hub9.sh")
echo "$submit_out"
