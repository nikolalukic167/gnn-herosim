#!/usr/bin/env bash
# argmax_uniq probe on one bipartite-coordination config (intra-batch platform mask).
# Usage: run_bipartite_uniq_platform_one.sh <config_name>
set -euo pipefail
...
LOG_DIR="${ROOT}/logs/bipartite_uniq_platform"
OUTPUT="${OUT_DIR}/${CONFIG_NAME}_gnn_dim22_argmax_uniq.json"
...
echo "Starting argmax_uniq ${CONFIG_NAME} -> ${OUTPUT} (log: ${LOG})"
...
srf = res / '${CONFIG_NAME}_gnn_dim22_seq_reforward.json'
if srf.is_file():
    baseline['seq_reforward'] = json.loads(srf.read_text()).get('total_rtt')
new = json.loads(Path('${OUTPUT}').read_text()).get('total_rtt')
print('DONE', '${CONFIG_NAME}', 'argmax_uniq=', new)
