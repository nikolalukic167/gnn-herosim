#!/usr/bin/env bash
# contention_v3 live gate (node_disk_v2, workload-125-225).
# Usage: run_contention_v3_live_gate_one.sh <knative|mlp|gnn> <config_name> <config_json_path>
set -euo pipefail
...
GNN_MODEL="${GNN_MODEL:-models/near-rtt-v2-contention-v3-dim14-ce-only.pt}"
MLP_MODEL="${MLP_MODEL:-models/tabular/batch_edge_mlp_contention_v3_dim22_batchcache.pt}"
...
echo "=== contention_v3 live gate: ${POLICY} / ${CONFIG_NAME} ==="
