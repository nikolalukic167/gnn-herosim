...
|---|---|---|
| [**route_b_env_pivot_v1**](docs/lineages/route_b_env_pivot_v1.md) | `ACTIVE` | Current work. Screen registered 2026-08-27. `greedy_stuck` was **decoder myopia on every rung** (458/458 rescued), not a configuration artifact; **AMENDMENT 2 signed off 2026-08-27** replaced the decoder and **H0/H1 counters are now clean** (α=2.0, α=3.0), H2 still VOID-GENERATION. Both controls then generated under AMENDMENT 1 and **both PASS S0**. Bars read 2026-08-27: **H0 VOID-TIE-INDETERMINATE on S1**, **H1 FAILS S1**. **S2 (the kill bar) is uncomputable on this grid** — `t1x` needs ≥82 sweep rows, the scarcity squeeze gives 16 and 64, refused 204/204. S3/S4 were blocked only by a **transfer-tool defect** (one arm's saturation refusal aborted the whole run); fixed, so **S4 PASSES both rungs** (`hop+coupling` closes 0.0000, full stratum) and **S3 lands on the bar** (H0 0.500000, H1 0.500835 vs `≤ 0.5`). Ladder **not** exhausted (H2 VOID-GENERATION, H3 never generated). S2 was then measured to be a **grid** problem, not a competitor problem — a 204/204 wide-arm probe fits `t1x` on 41/41 firing datasets with **zero saturation in both arms**, contention binding *harder* (0.91) and the squeeze untouched (2 hosting nodes everywhere) — and **H3 generates 0/204 as registered** (8 tasks, uniqueness-exhausted on a pool of 2 or 4). **AMENDMENT 3** moves the H2/H3 grid only and leaves H0/H1 alone. The pair it proposes was probed at 4 tasks (204/204) and **passes all four bars** (S1 0.2843 reg / 0.3137 pess, S2 58/58 fitted 0 saturated, S3/S4 0.0000) — so signing it likely yields the first **PIVOT-CANDIDATE**. Probe, not a rung: S0's control is ungenerated and the rung gets fresh seeds. **AMENDMENT 3 SIGNED OFF 2026-08-28** — H2/H3 move to `per_server` 4/5, H2 on fresh seeds 3401–3417, H3's skip threshold re-derived to 3e10. **The amended H2 was then generated and is VOID: its paired separable control FAILS S0** — `optimistic` **0.4853** vs a `≤ 0.02` bar at the primary α=2.0 (band 0.4853–0.8333, max 59.5%). Both corpora 204/204 clean, `{1680: 102, 3024: 102}`, squeeze intact `{2: 204}`. Not the tie artifact, not arm-confounded (per-arm 0.6765/0.6471, zero censoring), **not the scorer** (independent verifier 1e-9 on all 612 cells). **S1–S4 not read and must not be.** The failure appears only where the *cap* excludes the componentwise plan — unconstrained the band is exactly 0.0000, and where overlap's uniqueness alone excludes it (87/204) the surrogate still recovers the optimum every time. **Tightness is not the discriminator**: H1's control passes at `cw_infeas` 0.963 while H2's fails at 0.877. `node_caps` is plan-independent, so the evidence points at non-additive cost in the control arm, **mechanism unresolved**. **H3 generates** (40,320 rows, pool-8 arm, 0 skips) where as registered it gave 0/204 — the amendment's other half confirmed. Now open and bigger than one rung: **is S0 readable on any `replica_overlap` rung at all?** H2/H3 are the only two, and H3 inherits H2's grid. The earlier two open questions closed without action: the S0 rule is moot, S3's knife-edge stays. **The obvious diagnosis — re-run the control with `replica_overlap` OFF — is NOT RUNNABLE** (2026-08-28, 24-dataset probe, both arms): overlap is the flag that disables the allocator's exclusivity check, so turning it off moves pool size, per-type asymmetry and node confinement together — `per_server=4` gives `(8,4,1,1)`, `per_server=5` gives `(9,4,0,1)` where **`rf` starves to ZERO** and 12/24 die as `System state capture FAILED` (FAILED, not SKIPPED — its own GATE TOOLS row). And the arm that does generate yields **32-row sweeps** against H2's 1,680/3,024, i.e. the H0/H1 regime that already PASSES S0, so it is **uninformative on both arms**. Pre-registered NO-GO honoured: no corpus, no S0 read, grid not rescued by lowering `per_server`. Arm S integrity re-verified independently: **204/204 `sweep_complete`**, `{1680: 102, 3024: 102}`, 0 skips, 0 worker failures/timeouts. That direct diagnosis was then **run, and it resolves the mechanism**: regressing `rtt` on one-hot `(task, platform)` over the exhaustive sweep — no decoder, cap or surrogate in the path — gives **H2 ctrl R² median 0.78/0.82, residual 12.8%/13.8% of mean rtt**, against **H0/H1 ctrl R² 0.99997+, residual 0.16–1.0%**. The controls that PASS S0 fit almost exactly; the one that fails does not fit at all, flat across both arms. **MEASURED: the H2 control's cost is NOT additive** — the decode path is exonerated and the `separable-control-was-never-separable` family has recurred in a regime AMENDMENT 1 never tested. The residual is **co-residency-shaped**: at full co-location the additive model over-predicts by **−21.7%** (H0/H1: ≈0.0%), and a pairwise same-node term lifts R² to only ~0.92–0.95, so about half the coupling is higher-order. This puts the VOID on the **control definition** (`HEROSIM_STORAGE_NEUTRAL` + locality-off is not separable at H2's co-location densities) — amendment territory, not licensed. ⚠ A finding about the CONTROL, not evidence for the pivot: Arm S's bars remain unread. **AMENDMENT 4 is DRAFTED, NOT SIGNED OFF** (`screen-amendment-4.md` @ `d79ea54`): S0 would measure additivity **directly** — median per-dataset R² ≥ 0.99 and median residual ≤ 2.0%, per arm — instead of inferring it through the decoder. Thresholds calibrated off the two rungs that already PASS (worst passing arm: R² 0.999757, residual 0.999%), so it **changes no verdict anywhere** (H0 pass, H1 pass, **H2 still fail**) and does **not** unVOID H2; it makes S0 attributable, which is what cost a session here. Tool `scripts_cosim/measure_route_b_additivity.py` (no scorer imports, 8 tests); calibration frozen at `simulation_data/route_b_pivot_additivity_controls.json`. The alternative — fix the control's physics and regenerate — is documented in its §8 and not proposed, because the surviving coupling term has not been identified. **H3's control then generated 204/204 clean** (`{40320: 102, 362880: 102}` — §3's 8P8 **and** 9P8 measured, 0 skips, 0 worker failures; AMENDMENT 3's other half fully confirmed, where as registered H3 gave 0/204) **and it is NON-ADDITIVE TOO, worse than H2 by R²**: median **0.3568 / 0.5258**, residual 9.54% / 10.50%, pairwise lifting only to 0.655 / 0.756 (datalab job 719077). ⚠ Its co-residency breakdown is **DEGENERATE** — 8 tasks in a pool of 8–9 over 2 nodes gives only two occupancy levels, both reading 0.000%, so H2's co-location *shape* is **not** established on H3; only the magnitude is. The tool now flags this rather than reporting a zero. **Both overlap rungs fail separability**, so the ladder stands at **H0 VOID-TIE-INDETERMINATE / H1 FAIL / H2 VOID / H3 VOID — no PIVOT-CANDIDATE.** ⚠⚠ That is "**the screen could not measure it**", **NOT** "there is no exploitable joint structure" — Arm S's bars have never been read on either overlap rung and the same grid shape passed all four in probe. Do not collapse those two. |
