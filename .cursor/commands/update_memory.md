---
description: Update the memory/memory.md file with strict versioning and constraints.
---
...
2.  **Compare** the "Active Context" in the file vs. what we just accomplished.
3.  **Update** the file using **edit** operations (do not rewrite the whole file if possible, just the sections that changed).
...
5.  **Update** the `**Version:**` and `**Last Updated:**` - with hours and minutes not only days - at the top.
