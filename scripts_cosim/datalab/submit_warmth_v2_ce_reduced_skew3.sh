#!/usr/bin/env bash
# Submit 6-run warmth-v2 ce-reduced skew3 sweep: 3 configs × MLP + GNN (1 GPU each).
# Knative baseline already in warmth_v2_physics_skew3_20260611 — reuse for compare.
set -euo pipefail
...
gpu_job=$(sbatch \
  --job-name=wv2cr-gpu \
  --output="/home/nikola.lukic/gnn-herosim/logs/wv2cr-gpu-%A_%a.out" \
  --error="/home/nikola.lukic/gnn-herosim/logs/wv2cr-gpu-%A_%a.err" \
  --partition="$PARTITION" \
  --gres="$GRES" \
  --array=0-5 \
  --export=ALL,SWEEP_DIR,WORKLOAD,HEROSIM_WARMTH_PHYSICS,GNN_MODEL,MLP_MODEL \
  scripts_cosim/datalab/warmth_v2_ce_reduced_skew3_gpu.sbatch | awk '{print $NF}')

echo ""
echo "Submitted MLP/GNN array: ${gpu_job} (tasks 0-5)"
echo "Logs: logs/wv2cr-gpu-${gpu_job}_*.out"
echo "Monitor: squeue -u \$USER -n wv2cr-gpu"
echo "Knative baseline: simulation_data/normal_sim_sweeps/warmth_v2_physics_skew3_20260611/results/*_knative.json"
