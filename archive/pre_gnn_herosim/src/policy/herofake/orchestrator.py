...
class HROOrchestrator(Orchestrator):
    def initialize_state(self) -> HROSystemState:
        # Initialize scheduler state
        scheduler_state = HROSchedulerState(
        ...
        for task_type_name, task_type in self.data.task_types.items():
            # TODO: documentation
            task_platform_values: Dict[str, PlatformVector] = {
                metric: {
                    platform: task_type[metric][platform]
                    for platform in task_type["platforms"]
                }
                for metric in ["executionTime", "energy"]
            }

            task_platform_values["price"] = {
                platform: self.data.platform_types[platform]["price"]
                for platform in task_type["platforms"]
            }
            ...
            comparison["price"] = {
                platform: (
                    self.data.platform_types[baseline_platform]["price"]
                    / self.data.platform_types[platform]["price"]
                )
                for platform in task_type["platforms"]
            }
            ...
            scheduler_state.target_concurrencies[task_type_name] = {
                platform: (
                    ...
                    * sum(
                        ...
                        for metric in ["executionTime", "energy", "price"]
                    )
                )
                for platform in task_type["platforms"]
            }
            ...
            """
            logging.error(
                f"{task_type['name']} target_concurrencies = {target_concurrencies}"
            )
            """
    ...
    def monitor_process(self):
        ...
        while True:
            ...
            system_state: HROSystemState = yield self.mutex.get()
