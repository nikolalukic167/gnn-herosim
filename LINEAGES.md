...
| [**link_mp_v1**](docs/lineages/link_mp_v1.md) | `REGISTERED` | **Registered 2026-08-30, before any arm existed — the decisive experiment `mp_ablation_v1` called for.** Builds the link-aware graph (pre-existing `core_v1` network entities: physical nodes + core links + route edges, never live-gated) and re-runs the MP ablation on a fresh **binding-backbone** training corpus (1,800 target datasets, 3 fabric variants matching the live gate families; not `topo_transfer_v1`'s corpus, whose 1000 MB/s backbone made every link feature label-irrelevant). Three 16-seed arm families off one corpus — `lgon` (MP over the link graph), `lgctrl` (MP over the old graph), `lgmpoff` (no MP) — gated on the 20 backbone cells vs frozen Knative. Primary: one-sided exact Wilcoxon, H1 = lgon beats lgmpoff; S1 attributes any win to the link entities; verdicts and consequences fixed in the node and in `score_link_mp_v1.py` (18 tests). Live serving of a `core_v1` checkpoint smoke-verified end-to-end 2026-08-30 (first time ever run). |
...
| [**route_b_v1**](docs/lineages/route_b_v1.md) | `PARKED` | **Stage 1 PASS** — contention + coupling produces the non-pointwise structure five mechanisms and route A could not. **Stage 2 NO-GO-PREPROBE** (2026-08-26): a GNN cannot beat pointwise-plus-prefix on this environment even at memorization. Forked to `route_b_env_pivot_v1` — **that fork was PARKED 2026-08-28**, so this lineage parks with it; both measured results stand. |
