...
def build_valid_combos_map(placement_rtt_hash_table: Dict[Tuple[str, Tuple[Tuple[int, int], ...]], float]) -> Dict[str, List[Tuple[Tuple[int, int], ...]]]:
    """
    Build a simple lookup: dataset_id -> list of valid placement combos.
    Each combo is a tuple of (node_id, platform_id) pairs sorted by task_id.
    """
    from collections import defaultdict

    valid_map: Dict[str, List[Tuple[Tuple[int, int], ...]]] = defaultdict(list)
    for (ds_id, combo), _ in placement_rtt_hash_table.items():
        valid_map[ds_id].append(combo)
...
def decode_predicted_placement(logits_per_task, data):
    ...
    Args:
        ...
        constrain_to_simulated: If True, only return placements that exist in placement_rtt_hash_table
        placement_rtt_hash_table: Dictionary mapping (dataset_id, combo) -> RTT (required if constrain_to_simulated=True)
    """
    n_tasks = data.n_tasks
...
@torch.no_grad()
def decode_constrained_placement(logits_per_task, data, valid_combos_map):
    """
    Constrained decoding (Option 2):
    Instead of argmax per task, score only VALID simulated combinations
    from placements.jsonl and return the combo with highest total score.

    Returns:
        best_combo: Tuple[(node_id, platform_id), ...] or None
    """
    dataset_id = getattr(data, 'dataset_id', None)
    if not dataset_id or dataset_id not in valid_combos_map:
        return None
    ...
    # Map graph platform positions -> platform_ids
    plat_pos_by_id = getattr(data, '_plat_pos_by_id', {})
    if not plat_pos_by_id:
        return None
    plat_id_by_pos = {pos: pid for pid, pos in plat_pos_by_id.items()}

    ei = data.edge_index
    if ei.numel() == 0:
        return None
    ti = ei[0]                      # task indices
    pj = ei[1] - n_tasks            # platform positions

    # Precompute per-task platform_id -> score map
    task_plat_scores: List[Dict[int, float]] = []
    for t in range(n_tasks):
        mask_t = (ti == t)
        pj_t = pj[mask_t]
        logits_t = logits_per_task[t]
        if logits_t.numel() == 0 or logits_t.numel() != pj_t.numel():
            task_plat_scores.append({})
            continue

        scores: Dict[int, float] = {}
        for idx, plat_pos_tensor in enumerate(pj_t):
            plat_pos = int(plat_pos_tensor.item())
            plat_id = plat_id_by_pos.get(plat_pos)
            if plat_id is None:
                continue
            scores[int(plat_id)] = float(logits_t[idx].item())
        task_plat_scores.append(scores)

    valid_combos = valid_combos_map[dataset_id]
    if not valid_combos:
        return None
    ...
    for combo in valid_combos:
        # combo: ((node_id, plat_id), ...) sorted by task_id
        if len(combo) != n_tasks:
            continue

        total = 0.0
        possible = True
        for task_idx, (_, plat_id) in enumerate(combo):
            score_map = task_plat_scores[task_idx] if task_idx < len(task_plat_scores) else {}
            if plat_id not in score_map:
                possible = False
                break
            total += score_map[plat_id]
...
def decode_inference_placement(logits_per_task, data):
    """
    Global greedy decoder for inference / evaluation:
    - Builds a single placement by selecting edges (task -> platform)
      in descending score order.
    - Enforces:
        1) Each task is assigned at most once.
        2) Each platform position is used by at most one task (uniqueness).
    - Relies on graph construction to have already filtered to replica-backed,
      type-compatible, network-feasible edges.

    Returns:
        combo_tuple: Tuple[(node_id, platform_id), ...] sorted by task_id, or None.
    """
    dataset_id = getattr(data, 'dataset_id', None)
    if not dataset_id:
        return None
    ...
    # Edge index unpacking (must mirror model.forward)
    ei = data.edge_index
    if ei.numel() == 0:
        return None

    ti_full = ei[0]
    pj_full = ei[1] - n_tasks
    valid = (pj_full >= 0) & (pj_full < data.n_platforms)
    ti = ti_full[valid]
    pj = pj_full[valid]

    # Collect all candidate edges as (score, task_idx, platform_pos)
    ...
    for t in range(n_tasks):
        ...
        for idx, plat_pos_tensor in enumerate(pj_t):
            plat_pos = int(plat_pos_tensor.item())
            score = float(logits_t[idx].item())
            candidates.append((score, t, plat_pos))
    ...
    assigned_tasks = set()
    used_platforms = set()
    final_pairs: List[Tuple[int, int]] = []  # (task_idx, plat_pos)

    for score, t_idx, plat_pos in candidates:
        if t_idx in assigned_tasks:
            continue
        if plat_pos in used_platforms:
            continue
        assigned_tasks.add(t_idx)
        used_platforms.add(plat_pos)
        final_pairs.append((t_idx, plat_pos))
        if len(assigned_tasks) == n_tasks:
            break

    if len(assigned_tasks) < n_tasks:
        # Could not assign all tasks under uniqueness constraint
        return None

    # Map (task_idx, plat_pos) -> (node_id, platform_id), sorted by true task_id
    plat_pos_by_id = getattr(data, '_plat_pos_by_id', {})
    if not plat_pos_by_id:
        return None
    plat_id_by_pos = {pos: pid for pid, pos in plat_pos_by_id.items()}

    task_idx_to_task_id = getattr(data, '_task_idx_to_task_id', {})
    plat_node_map = DATA_PLAT_NODE_MAP.get(dataset_id, {})

    # Sort by actual task_id (keys in hash table combos are ordered by task id)
    final_pairs.sort(key=lambda x: task_idx_to_task_id.get(x[0], x[0]))

    combo_list: List[Tuple[int, int]] = []
    for t_idx, plat_pos in final_pairs:
        plat_id = plat_id_by_pos.get(plat_pos)
        if plat_id is None:
            return None
        node_id = plat_node_map.get(int(plat_id))
        if node_id is None:
            return None
        ...
        for data in graphs_in_batch:
            ...
            if valid_ce > 0:
                ...
                try:
                    ...
                    if dataset_id and dataset_id in DATA_OPTIMAL_RTT and 'placement_rtt_hash_table' in globals():
                        # Inference-style decoding: construct a single valid combo greedily
...
# Build valid combos map for constrained decoding (Option 2)
