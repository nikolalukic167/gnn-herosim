...
| [**rollout_imitation_v1**](docs/lineages/rollout_imitation_v1.md) | `ACTIVE` | One step of policy improvement on the simulator over the immediate rule (forced-placement rollouts, group-local cost), with a blocking rank-stability control against horizon chaos. Triggered by `joint_burst_v2` (CD greedy is the ceiling). Build started 2026-09-21: label engine → Phase A rank-stability → A.2 label-vs-rule gain (both blocking) before any training. |
