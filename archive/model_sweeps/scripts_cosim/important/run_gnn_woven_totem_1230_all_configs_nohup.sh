...
OUT_ROOT="${1:-simulation_data/normal_sim_sweeps/gnn_woven_totem_1230_20260608}"
TS="$(basename "$OUT_ROOT" | grep -oE '[0-9]{8}(_[0-9]{6})?' | tail -1 || date +%Y%m%d_%H%M%S)"
CFG_DIR="${OUT_ROOT}/configs"
RES_DIR="${OUT_ROOT}/results"
PROGRESS_LOG="logs/gnn_woven_totem_1230_sweep_${TS}.log"
...
log "=== GNN woven-totem-1230 sweep start ==="
...
log "=== GNN woven-totem-1230 sweep complete ==="
