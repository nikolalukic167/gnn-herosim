#!/usr/bin/env bash
# One bipartite v2 job with skew-merged models + node_disk_v2 physics.
# Usage: run_bipartite_skew_merged_one.sh <gnn|mlp> <config_name> <config_json_path>
#
...
# Knative baselines: already in regime_b_hub9_20260612/results/ (Kn + KnECT x 9 configs).
# Workload: workload-125-225.json (562k tasks, bipartite stress).
set -euo pipefail
...
OUT_DIR="${SWEEP_DIR:-simulation_data/normal_sim_sweeps/bipartite_v2_skew_merged_20260614}/results"
SEED="${SEED:-42}"
...
[[ -f "$MODEL_CHECK" ]] || { echo "ERROR: model missing: $MODEL_CHECK" >&2; exit 1; }
...
echo "=== bipartite v2 skew-merged (node_disk_v2): ${POLICY} / ${CONFIG_NAME} ==="
