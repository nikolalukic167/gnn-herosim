...
OUT_ROOT="${1:-simulation_data/normal_sim_sweeps/gnn_near_rtt_v2_dim14_ce_init_regret_$(date +%Y%m%d)}"
MODEL_PATH="${2:-models/near-rtt-v2-dim14-ce-init-regret.pt}"
TS="$(basename "$OUT_ROOT" | grep -oE '[0-9]{8}(_[0-9]{6})?' | tail -1 || date +%Y%m%d_%H%M%S)"
CFG_DIR="${OUT_ROOT}/configs"
RES_DIR="${OUT_ROOT}/results"
PROGRESS_LOG="logs/gnn_near_rtt_v2_dim14_ce_init_regret_sweep_${TS}.log"
...
log "=== GNN near-rtt-v2-dim14-ce-init-regret sweep start ==="
log "Model: ${MODEL_PATH}"
log "Decode: argmax (no LQB)"
log "Output: ${OUT_ROOT}"
...
log "=== GNN near-rtt-v2-dim14-ce-init-regret sweep complete ==="
