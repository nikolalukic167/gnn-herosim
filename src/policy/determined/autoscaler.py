...
class DeterminedAutoscaler(Autoscaler):





    def scaling_level(self, system_state: DeterminedSystemState, task_type: TaskType):
    ...
    def initialize_replica(
        ...
        if needs_image_pull(physics, platform, node, task_type):
            ...
            # Re-check after FilterStore handoff — peer pull may have cached image (node_disk_v2)
            if needs_image_pull(
