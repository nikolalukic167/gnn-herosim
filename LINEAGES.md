...
|---|---|---|
| [**joint_burst_v2**](docs/lineages/joint_burst_v2.md) | `ACTIVE` | Why `joint_burst_v1` lost to the greedy: the model was served CAPPED out of the label's move on loaded states its corpus REJECTED, and a coordinate-descent greedy cuts the 1-pass greedy's regret 33.5→11.2 s. v2 keeps the loaded states (2,038+480), trains/serves UNCAPPED, gates vs CD (honest) and 1-pass (reachable) greedies. |
| [**rollout_imitation_v1**](docs/lineages/rollout_imitation_v1.md) | `REGISTERED` | One step of policy improvement on the simulator over the immediate rule (forced-placement rollouts, group-local cost), with a blocking rank-stability control against horizon chaos; `joint_burst_v1` read NO-GNN-WIN with J5 SERVED-DISTRIBUTION-HELPS — this is the pre-signed next lever, build not yet started. |
