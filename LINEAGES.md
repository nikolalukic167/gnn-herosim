...
| [**reliability_matched_v1**](docs/lineages/reliability_matched_v1.md) | `REGISTERED` | **Registered 2026-09-03, before any gate episode.** Does the GNN's severe-collapse reliability edge survive training BOTH classes on the same corpus? Phase 1 established it across *different* corpora, and the 2026-09-03 latency pilot showed the latency half of that same pairing was entirely a corpus effect (`link_mp_v1`). Design: the 16 frozen `lgon` GNN arms (0 collapses in 16x20 cells) vs 16 fresh MLP seeds on the GNN's own cache, on link_mp_v1's 20 backbone cells, with 4 GNN arms re-gated at HEAD as a venue control that can VOID the gate. Statistics inherited verbatim from the signed Phase 1 registration (rank-sum at +50% primary, +100% must-hold, +30% descriptive, alpha 0.05). All four outcomes — including TIE-AT-ZERO, which would mean reliability was the corpus too — written before the data. |
