...
|---|---|---|
| [**objective_pivot_v1**](docs/lineages/objective_pivot_v1.md) | `ACTIVE` | **Current work. Program pivot registered 2026-08-28: stop engineering the environment, change the training objective.** Phase 1: a properly powered **draw-distribution** reliability gate — extend `gnn_draw_study_v1` to 16 seeded draws (P5b already ran and is CLOSED: feature null, collapse draw-dominated; the claim shape is "tight distribution with a tail vs a lottery", never "never collapses"). **Phase 1 registration SIGNED OFF 2026-08-28** with a binding venue/code pin: new arms run at the frozen arms' recorded commit `c08aa7e` (clean tree, verified from cluster `run_provenance`), scorer-asserted per arm, mismatch ⇒ VOID. Not yet run. Phase 2: the P3 horizon pilot as registered in `program_verdict_v1`. Phase 3: P1 closed-loop training (DAgger/policy-gradient against the live simulator, paired-seed baseline, fair closed-loop MLP arm, kill criterion registered before spend) — the one path `program_verdict_v1` left open to the latency claim. No phase's numbers exist yet. |
