...
"""CE-reduced MLP from scratch on merged warmth_v2 + sparse_warmth_v2 seq cache."""
...
def main() -> None:
    ...
    seq_cache = Path(
        os.environ.get(
            "MLP_SEQ_CACHE_DIR",
            str(repo / "simulation_data" / "graphs_cache_warmth_v2_sparse_merged_seq"),
        )
    )
    ...
    model = Path(
        os.environ.get(
            "MLP_MODEL_PATH",
            str(repo / "models" / "tabular" / "batch_edge_mlp_warmth_sparse_merged_ce_reduced.pt"),
        )
    )
