#!/usr/bin/env bash
# Push hub9 MLP ablation (18 jobs) to datalab and submit.
set -euo pipefail
...
MLP_CE_REDUCED="models/tabular/batch_edge_mlp_warmth_sparse_merged_ce_reduced.pt"
...
for f in "$MLP_CE_REDUCED" "$MLP_DIM22" "$WORKLOAD" "$SKEW_BASE"; do
  if [[ ! -f "$f" ]]; then
...
bash scripts_cosim/important/prepare_warmth_sparse_merged_hub9_ablation_sweep.sh

job_count=$(($(wc -l < "${SWEEP_DIR}/configs/jobs_smcr_hub9_ablation.tsv") - 1))
if [[ "$job_count" -ne 18 ]]; then
  echo "ERROR: expected 18 ablation jobs, got ${job_count}" >&2
  exit 1
fi
...
rsync -avP -e "$RSYNC_SSH" "$MLP_CE_REDUCED" "${REMOTE}:${REPO}/models/tabular/"
rsync -avP -e "$RSYNC_SSH" "${MLP_CE_REDUCED}.meta.json" "${REMOTE}:${REPO}/models/tabular/" 2>/dev/null || true
rsync -avP -e "$RSYNC_SSH" "$MLP_DIM22" "${REMOTE}:${REPO}/models/tabular/"

echo "[2/4] rsync workload + hub configs + ablation manifest..."
...
rsync -avP -e "$RSYNC_SSH" \
  scripts_cosim/datalab/run_warmth_sparse_merged_ce_reduced_hub9_ablation_one.sh \
  scripts_cosim/datalab/warmth_sparse_merged_ce_reduced_hub9_ablation_gpu.sbatch \
  scripts_cosim/datalab/submit_warmth_sparse_merged_ce_reduced_hub9_ablation.sh \
  "${REMOTE}:${REPO}/scripts_cosim/datalab/"
rsync -avP -e "$RSYNC_SSH" \
  scripts_cosim/important/prepare_warmth_sparse_merged_hub9_ablation_sweep.sh \
...
ssh -i "$SSH_KEY" "$REMOTE" "chmod +x \
  ${REPO}/scripts_cosim/datalab/run_warmth_sparse_merged_ce_reduced_hub9_ablation_one.sh \
  ${REPO}/scripts_cosim/datalab/submit_warmth_sparse_merged_ce_reduced_hub9_ablation.sh \
  ${REPO}/scripts_cosim/important/prepare_warmth_sparse_merged_hub9_ablation_sweep.sh && \
  sed -i 's/\r$//' ${REPO}/scripts_cosim/datalab/*.sh ${REPO}/scripts_cosim/datalab/*.sbatch"

echo "[4/4] submit on datalab..."
submit_out=$(ssh -i "$SSH_KEY" "$REMOTE" "cd ${REPO} && bash scripts_cosim/datalab/submit_warmth_sparse_merged_ce_reduced_hub9_ablation.sh")
echo "$submit_out"
...
echo ""
echo "Waiting 90s then checking queue..."
sleep 90

ssh -i "$SSH_KEY" "$REMOTE" "squeue -u nikola.lukic -j ${job_id} -o '%.10i %.12P %.16j %.2t %.10M %R' ; echo '---' ; squeue -u nikola.lukic -n wsmcr-hub9-abl-${partition#GPU-} -o '%.10i %.2t %.10M %R' 2>/dev/null || true"
