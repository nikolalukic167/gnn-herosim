...
**Status:** `REGISTERED` (2026-09-24) — P0 (live headroom gate) signed before any data; P1–P4
registered in outline, to be signed in full before P1 starts. Triggered by `joint_burst_v2`'s K4
tie and the 2026-09-24 reads recorded below.

**Standing answer (2026-09-24): not run.** The question is the one CLAUDE.md asks — does a
graph-aware scheduler beat its pointwise twin — narrowed to the one mechanism that is
architecturally graph-specific and has never been testable here: pricing partners that have
not arrived yet, through partners of partners. P0 asks the cheapest prior question first:
along the rule's own trajectory, is there live headroom in pricing unarrived partners at all?
If an oracle that knows where each partner will run cannot beat the rule by 5 %, no learned
model can carry a 5 % model-class gap through lookahead, and the lineage stops at P0.
