...
|---|---|---|
| [**peer_affinity_v1**](docs/lineages/peer_affinity_v1.md) | `ACTIVE` | **PIVOT-CANDIDATE for training (2026-09-10)** — the first environment whose joint structure survives a correctly specified count competitor, on paper AND in the simulator. A cost indexed by *pairs of task instances* (continuous exchange volumes, no commit order) under a binding cap. Paper screen GO (14/60 cells); physics `HEROSIM_PEER_EXCHANGE=1` built, bit-identical off; simulated rung R2 read blind on fresh seeds (34 datasets, k = 10, 200 MB, α₁₀ = 2.5): every registered bar passes — pointwise regret 28.5 %, count repair 0.27 (residual 23.4 %), greedy analogues 33 % / 20 %, count oracle 19 %, control arm exact (0/34 > 1 %), cap binds 34/34. 50 MB cell dropped (fails B1 on the simulator). Six pre-read amendments in the node. **Next: training registration (separate sign-off).** `scripts_cosim/peer_affinity_probe.py`. |
