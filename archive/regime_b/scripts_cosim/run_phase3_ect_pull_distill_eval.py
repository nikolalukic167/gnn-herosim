#!/usr/bin/env python3
"""
Phase 3: evaluate ect_pull-distilled GNN on oracle_split_v1.

Primary decode = seq_reforward_pull (cross-batch ledger; matches teacher).
Also reports argmax control + CE baseline + ect_pull ceiling.

Primary metric = regime_b_primary_score_s (max burst elapsed).
...
DEFAULT_OUT = (
    PROJECT_ROOT
    / "simulation_data/normal_sim_sweeps/regime_b_phase3_ect_pull_distill_eval"
)
...
def _load_gnn(model_path: Path, *, label: str) -> Dict[str, Any]:
    if not model_path.is_file():
        raise FileNotFoundError(f"FAIL LOUD: {label} GNN missing: {model_path}")
    ...
    if plat_in < 16:
        raise RuntimeError(
            f"FAIL LOUD: Phase 3 requires dim16 platform encoder (got {plat_in}). "
...
def main() -> int:
    ...
    parser.add_argument(
        "--modes",
        default="seq_reforward_pull,argmax",
        help="Comma-separated distill decode modes (primary=seq_reforward_pull)",
    )
    ...
    print("=" * 72)
    print(f"Phase 3 ect_pull distill eval — {PROBLEM_ID} / {INTEL_STUB_VARIANT}")
    print(f"stub={stub_dir}")
    print(
        f"distill={distill_models['model_path']}  "
        f"md5={distill_models['model_md5'][:12]}…  "
        f"plat_in={distill_models['platform_in_features']}"
    )
    print(f"layout=dim24  N={TARGET_N_TASKS}")
    print("=" * 72)

    summary: Dict[str, Any] = {
        "phase": "phase3_ect_pull_distill_eval",
        ...
        "platform_in_features": distill_models["platform_in_features"],
    ...
    if not args.skip_refs:
        ...
        print("\n--- reference: ect_pull (teacher ceiling) ---")
        ect = run_policy(
            sim_inputs,
            infra,
            workload,
    ...
    if not args.skip_ce_baseline:
        ...
        print(f"  {PRIMARY_SCORE_KEY}={ce_scored[PRIMARY_SCORE_KEY]:.2f}s")
    ...
    for mode in modes:
        print(f"\n--- distill decode={mode} ---")
        scored = _run_gnn(
            sim_inputs, infra, workload, models=distill_models, decode_mode=mode
        )
    ...
    primary_key = "distill_seq_reforward_pull"
    if primary_key in summary["policies"]:
        joint = float(summary["policies"][primary_key][PRIMARY_SCORE_KEY])
        soft_argmax = None
        if "distill_argmax" in summary["policies"]:
            soft_argmax = float(summary["policies"]["distill_argmax"][PRIMARY_SCORE_KEY])
        ce_floor = None
        ...
        if near_ceiling:
            interpretation = (
                "ect_pull distill + seq_reforward_pull recovers physics ceiling"
            )
        elif still_pile:
            interpretation = (
                "hostile — distill failed to transfer ect_pull trajectory "
                "(stayed in 90–125s pile band)"
            )
        elif vs_ce is not None and vs_ce > 5.0:
            interpretation = (
                "partial — beats CE but residual to ect_pull remains"
            )
        else:
            interpretation = "null / incomplete — distill did not close Regime B residual"

        summary["falsification"] = {
            "distill_seq_reforward_pull_primary_s": joint,
            "distill_argmax_primary_s": soft_argmax,
        ...
        print(f"  distill seq_reforward_pull: {joint:.2f}s")
        if soft_argmax is not None:
            print(f"  distill argmax:            {soft_argmax:.2f}s")
