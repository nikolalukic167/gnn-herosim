#!/usr/bin/env bash
# Push sparse-merged ce-reduced skew3 sweep + models to datalab.
set -euo pipefail
...
SWEEP_DIR="${SWEEP_DIR:-simulation_data/normal_sim_sweeps/warmth_sparse_merged_ce_reduced_skew3_20260611}"
...
echo "[1/3] rsync sparse-merged ce-reduced models..."
...
rsync -avP -e "$RSYNC_SSH" \
  scripts_cosim/datalab/run_warmth_sparse_merged_ce_reduced_skew3_one.sh \
  scripts_cosim/datalab/warmth_sparse_merged_ce_reduced_skew3_gpu.sbatch \
  scripts_cosim/datalab/submit_warmth_sparse_merged_ce_reduced_skew3.sh \
...
ssh -i "$SSH_KEY" "$REMOTE" "chmod +x \
  ${REPO}/scripts_cosim/datalab/run_warmth_sparse_merged_ce_reduced_skew3_one.sh \
  ${REPO}/scripts_cosim/datalab/submit_warmth_sparse_merged_ce_reduced_skew3.sh"

echo "Transfer complete -> ${REMOTE}:${REPO}"
echo "Submit on datalab: bash scripts_cosim/datalab/submit_warmth_sparse_merged_ce_reduced_skew3.sh"
