#!/usr/bin/env bash
# Push merged warmth+sparse JSONL BF finish to datalab and submit.
set -euo pipefail
...
rsync -avP -e "$RSYNC_SSH" \
  ...
  scripts_cosim/datalab/submit_warmth_merged_jsonl_finish.sh \
...
echo "[+] Merged JSONL finish scripts synced."
echo "    Submitting on datalab..."
ssh -i "$SSH_KEY" "$REMOTE" \
  "cd ${REPO} && sed -i 's/\r$//' scripts_cosim/datalab/*.sh scripts_cosim/datalab/*.sbatch scripts_cosim/*.sh && \
  bash scripts_cosim/datalab/submit_warmth_merged_jsonl_finish.sh"
