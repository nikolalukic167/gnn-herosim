...
|---|---|---|
| [**peer_affinity_v1**](docs/lineages/peer_affinity_v1.md) | `ACTIVE` | **Phase 0 paper screen GO (2026-09-10)** — the first joint structure in this program that per-machine count columns do not repair. A cost indexed by *pairs of task instances* (continuous exchange volumes x_ij, no commit order) under a binding cap: 14 of 60 paper cells pass every registered bar on 34/34 sources; in the GO cell (`x50_a1.5_k10c3_p2`: k = 10, 3 candidates, 2 partners, α₁₀ = 3.75) the count competitor with an exhaustive decode keeps 17.6 % regret (repair 0.31), the prefix greedy 49 %, the hand-lookahead greedy 31 % (8 % under the best deterministic ordering). Controls additive (S0b R² = 1.0000 in all 60 cells); cross-check vs `separability_diagnostic` 68/68. Deciding bar (count repair) is near-threshold in most cells — A2 must reproduce it on simulated RTTs before training. Three pre-screen amendments (base cost, equal-tightness α_k, tie band) in the node. Next: build `HEROSIM_PEER_EXCHANGE=1` (plan §A1) and the k = 10 grid (§A2). |
