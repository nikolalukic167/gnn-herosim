...
Called as: joint_burst_v2_sidecheck.py <contract.json> <arm: gnnedge0|mpoff|v4load|v4twin|bc1load|bc1mpoff|fc1load|xs1load> <split.json> <want_alpha>
...
def main() -> int:
    ...
    if arm in ("bc1load", "bc1mpoff", "fc1load", "xs1load"):
    ...
    if arm in ("fc1load", "xs1load"):
        want["full_context_ce_weight"] = 1.0
    if arm == "xs1load":
        want["exchange_seconds"] = True
    if arm in ("gnnedge0", "v4load", "v4twin", "bc1load", "fc1load", "xs1load"):
