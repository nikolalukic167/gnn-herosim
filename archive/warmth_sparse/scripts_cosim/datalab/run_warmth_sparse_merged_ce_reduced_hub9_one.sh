#!/usr/bin/env bash
# One sparse-merged ce-reduced hub9 job (node_disk_v2, 125-225): gnn | mlp | knative.
# Usage: run_warmth_sparse_merged_ce_reduced_hub9_one.sh <gnn|mlp|knative> <config_name> <config_json_path>
set -euo pipefail
...
case "$POLICY" in
  ...
  knative)
    ...
    ;;
  *)
    echo "ERROR: policy must be gnn, mlp, or knative; got: ${POLICY}" >&2
    exit 1
    ;;
esac
