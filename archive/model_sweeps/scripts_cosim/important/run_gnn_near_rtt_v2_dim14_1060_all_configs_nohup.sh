...
OUT_ROOT="${1:-simulation_data/normal_sim_sweeps/gnn_near_rtt_v2_dim14_1060_$(date +%Y%m%d)}"
TS="$(basename "$OUT_ROOT" | grep -oE '[0-9]{8}(_[0-9]{6})?' | tail -1 || date +%Y%m%d_%H%M%S)"
CFG_DIR="${OUT_ROOT}/configs"
RES_DIR="${OUT_ROOT}/results"
PROGRESS_LOG="logs/gnn_near_rtt_v2_dim14_1060_sweep_${TS}.log"
...
log "=== GNN near-rtt-v2-dim14-1060 sweep start ==="
...
log "=== GNN near-rtt-v2-dim14-1060 sweep complete ==="
