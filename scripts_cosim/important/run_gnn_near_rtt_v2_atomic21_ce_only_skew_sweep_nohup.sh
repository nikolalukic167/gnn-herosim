...
PROGRESS_LOG="logs/gnn_near_rtt_v2_atomic21_ce_only_sweep_${TS}.log"
...
MODEL_PATH="models/near-rtt-v2-atomic21-ce-only.pt"
TIMEOUT=3600
SEED=42
...
log "=== GNN near-rtt-v2-atomic21-ce-only skew sweep start ==="
...
if [[ ! -f "$SPARSE_UNIFORM" ]]; then
  log "ERROR: sparse uniform config missing: ${SPARSE_UNIFORM}"
  exit 1
fi
run_one "05_sparse_40_40_p25" "$SPARSE_UNIFORM" "${RES_DIR}/05_sparse_40_40_p25.json"
...
log "=== GNN near-rtt-v2-atomic21-ce-only skew sweep complete ==="
