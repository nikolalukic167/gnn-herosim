...
"""
CE-only near-RTT trainer with reduced v1 feature ablation (same cache, subset dims).

Keeps: task onehot + src_norm; platform type onehot + norm queue; edge exec + latency.
Omits: replica bits, shared_fate, temporal, tc/usage_ratio, is_warm, energy, comm_time.

Loads full graphs from cache then slices features in-process (no cache regen).
"""
...
sys.path.insert(0, str(_REPO_FOR_IMPORT / "src" / "notebooks"))
...
# CE-only + cache defaults (must run before parse_training_config).
...
os.environ.setdefault("WANDB_RUN_NAME", "near-rtt-ce-reduced-features")
os.environ.setdefault(
    "WANDB_TAGS",
    "near-rtt,ce-only,reduced-features,1060,ablation",
)
if "--cache-dir" not in sys.argv:
    sys.argv.extend(["--cache-dir", str(_DEFAULT_CACHE_DIR)])
if "--regret-loss-weight" not in sys.argv:
    sys.argv.extend(
        [
            "--regret-loss-weight",
            "0",
...
# Reduced layout (from full 14-d platform + 5-d edge cache).
...
def apply_reduced_features_to_graph(graph: Data) -> Data:
...
class TaskPlacementGNN(nn.Module):
    def __init__(
        ...
        edge_dim: int = REDUCED_EDGE_FEATURE_DIM,
    ) -> None:
        super().__init__()
...
def _refresh_queue_dependent_platform_features(
    ...
    if platform_features.size(-1) != REDUCED_PLATFORM_FEATURE_DIM:
        raise RuntimeError(
            f"Reduced sequential validation expects {REDUCED_PLATFORM_FEATURE_DIM}-dim platform features; "
            f"got {platform_features.size(-1)}."
        )
    ...
    for queue_key, info in meta.items():
        ...
        platform_features[pos, REDUCED_PLATFORM_QUEUE_DIM] = raw_q / float(queue_norm)
...
for _g in graphs:
    apply_reduced_features_to_graph(_g)
...
print(f"Loaded {len(graphs)} graphs (reduced features: task={REDUCED_TASK_FEATURE_DIM}, "
...
wandb.init(
    ...
    config={
        ...
        "reduced_task_dim": int(REDUCED_TASK_FEATURE_DIM),
        "reduced_platform_dim": int(REDUCED_PLATFORM_FEATURE_DIM),
        "reduced_edge_dim": int(REDUCED_EDGE_FEATURE_DIM),
...
model = TaskPlacementGNN(
    task_feature_dim=REDUCED_TASK_FEATURE_DIM,
    platform_feature_dim=REDUCED_PLATFORM_FEATURE_DIM,
    embedding_dim=EMBEDDING_DIM,
    hidden_dim=HIDDEN_DIM,
    num_layers=NUM_GIN_LAYERS,
    normalize_platform_inputs=False,
    edge_dim=REDUCED_EDGE_FEATURE_DIM,
).to(DEVICE)
