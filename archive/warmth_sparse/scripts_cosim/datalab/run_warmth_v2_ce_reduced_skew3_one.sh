#!/usr/bin/env bash
# One warmth-v2 ce-reduced skew3 job (node_disk_v2): mlp | gnn.
# Usage: run_warmth_v2_ce_reduced_skew3_one.sh <mlp|gnn> <config_name> <config_json_path>
set -euo pipefail
...
GNN_MODEL="${GNN_MODEL:-models/near-rtt-v2-warmth-ce-reduced.pt}"
MLP_MODEL="${MLP_MODEL:-models/tabular/batch_edge_mlp_warmth_ce_reduced.pt}"
OUT_DIR="${SWEEP_DIR:-simulation_data/normal_sim_sweeps/warmth_v2_ce_reduced_skew3_20260611}/results"
SEED=42
...
case "$POLICY" in
  gnn)
    OUTPUT="${OUT_DIR}/${CONFIG_NAME}_gnn_warmth_ce_reduced.json"
    export GNN_MODEL_PATH="${GNN_MODEL}"
    RUN_ARGS=(--gnn)
    ;;
  mlp)
    OUTPUT="${OUT_DIR}/${CONFIG_NAME}_mlp_warmth_ce_reduced.json"
...
echo "=== warmth-v2 ce-reduced skew3 ${POLICY} ${CONFIG_NAME} ==="
