...
OUT_ROOT="${1:-simulation_data/normal_sim_sweeps/gnn_woven_totem_1230_seqblend_m2_20260608}"
TS="$(basename "$OUT_ROOT" | grep -oE '[0-9]{8}(_[0-9]{6})?' | tail -1 || date +%Y%m%d_%H%M%S)"
CFG_DIR="${OUT_ROOT}/configs"
RES_DIR="${OUT_ROOT}/results"
PROGRESS_LOG="logs/gnn_woven_totem_1230_seqblend_m2_sweep_${TS}.log"
...
MODEL_PATH="models/woven-totem-1230.pt"
TIMEOUT=3600
SEED=42
...
log "=== GNN woven-totem-1230 seqblend m${SEQBLEND_MARGIN} sweep start ==="
...
log "=== GNN woven-totem-1230 seqblend m${SEQBLEND_MARGIN} sweep complete ==="
