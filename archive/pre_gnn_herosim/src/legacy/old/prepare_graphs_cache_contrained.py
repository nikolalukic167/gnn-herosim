...
CACHE_DIR = BASE_DIR.parent / "graphs_cache_old_with_queues"
...
def _parse_jsonl_file(jsonl_path: Path) -> Dict[Tuple[str, Tuple[Tuple[int, int], ...]], float]:
...
def build_and_save_rtt_hash_table_chunked(
    ...
    parsed_dicts: List[Dict] = Parallel(n_jobs=n_jobs, prefer="processes")(
        delayed(_parse_jsonl_file)(Path(p)) for p in tqdm(all_jsonl_files, desc="Parsing JSONL files")
    )
...
def build_graph(df_nodes, df_tasks, df_platforms, queue_snapshot: Optional[Dict[str, int]] = None) -> Data:
    ...
    # TASK FEATURES
    print(f"[build_graph] Building graph for dataset with {len(df_tasks)} tasks and {len(df_platforms)} platforms")
    ...
    # Simple logging about replica availability and queue stats
    num_replica_dnn1 = int(has_dnn1_arr.sum())
    num_replica_dnn2 = int(has_dnn2_arr.sum())
    queue_stats = f"queue_mean={queue_lengths.mean():.1f}, queue_max={queue_lengths.max():.0f}" if queue_snapshot else "no queue data"
    print(f"[build_graph] Replica summary: dnn1={num_replica_dnn1}, dnn2={num_replica_dnn2}, total_platforms={len(df_platforms)}, {queue_stats}")
    ...
    def filter_compatible_platforms(
        ...
        task_type: str,
        replicas_dnn1: np.ndarray,
        replicas_dnn2: np.ndarray
    ) -> np.ndarray:
        """
        Filter platforms by:
          1) Network feasibility (pre-filtered in network_feasible_plats)
          2) Task/platform type compatibility
          3) Replica existence for that task type (derived from system state)
        """
        ...
        if task_type == 'dnn1':
            ...
            replica_mask = replicas_dnn1
        elif task_type == 'dnn2':
            type_mask = plat_type_compat_dnn2
            replica_mask = replicas_dnn2
        else:
            return np.empty(0, dtype=np.int64)

        # Require BOTH: compatible type AND an actual replica for this task type
        combined_mask = type_mask & replica_mask
        compatible_mask = combined_mask[network_feasible_plats]
    ...
    for t_pos, (src_name, opt_pid, task_type) in enumerate(zip(src_names, optimal_platform_ids, task_types_arr)):
        ...
        compat_plats = filter_compatible_platforms(
            network_feas_plats,
            task_type,
            has_dnn1_arr,
            has_dnn2_arr,
        ...
        if compat_plats.size:
            ...
            # No feasible replica-backed platforms for this task; this should be rare
            print(f"[build_graph] WARNING: No replica-backed compatible platforms for task_id={df_tasks.iloc[t_pos]['task_id']} "
                  f"(type={task_type}, src={src_name}). Label set to -1.")
            y_list.append(-1)
...
def main():
    ...
    # Free memory from all_datasets (no longer needed)
    del all_datasets
